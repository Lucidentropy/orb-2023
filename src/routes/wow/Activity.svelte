<script lang="ts">
	...
	$: allActivities = activityRows().slice().reverse();
	...
	function activityParts(item: any) {
		...
		if (item.character_level_up) {
			return {
				type: 'text',
				text: `${item.character_level_up?.character?.name || 'Unknown'} reached level ${
					item.character_level_up?.level || '?'
				}`
			};
		}
		...
		if (item.item_looted) {
			return {
				type: 'text',
				text: `${item.item_looted?.character?.name || 'Unknown'} looted ${
					item.item_looted?.item?.name || 'an item'
				}`
			};
		}
...
<section class="space-y-4 h-full flex flex-col ">
	...
	<div class="overflow-hidden rounded border border-border-faint/60 bg-bg-deep/20">
        ...
        <div class="grid grid-cols-[30px_minmax(0,1.7fr)_120px_80px_minmax(0,1fr)] gap-3 border-b border-border-faint px-4 py-3 font-mono text-xs uppercase tracking-widest text-orb-highlight/40">
            <div>&nbsp;</div>
		</div>
		<div class="space-y-2 border-l-2 border-border-faint px-4 py-4">
					...
					<div class="rounded border border-border-faint/40 bg-bg-deep/20 px-4 py-3">
						<p class="mb-1 text-sm text-orb-highlight/80">
							{#if activityParts(item).type === 'achievement'}
                                ...
                                <a
                                    href={`https://www.wowhead.com/achievement=${activityParts(item).id}`}
                                    data-wowhead={`achievement=${activityParts(item).id}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    class="mx-1 font-semibold text-yellow-300 hover:text-white hover:underline"
                                    title={null}
                                >
                                    [{activityParts(item).achievement}]
                                </a>
							{:else}
